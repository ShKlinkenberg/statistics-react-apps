...
export default function NHSTExplainer() {
    ...
    const power = useMemo(() => {
        if (twoTailed)
            return 1 - normalCDF((zCritRight - muAlt) / se) + normalCDF((zCritLeft - muAlt) / se);
    ...
    function curveOpacity(which) {
        ...
        if (which === "null")
            return nullConcepts.includes(hovered) ? 1 : 0.3;
        if (which === "alt")
            return altConcepts.includes(hovered) ? 1 : 0.3;
        return 1;
    }
    ...
    return (<div style={{ fontFamily: "system-ui, sans-serif", maxWidth: 760, margin: "0 auto", padding: 16 }}>
      ...
      <svg width="100%" viewBox={`0 0 ${W} ${H}`} style={{ border: "1px solid #e5e7eb", borderRadius: 8, background: "#fafafa" }}>
        ...
        <polygon points={twoTailed ? fillRegion(0, zCritLeft, zCritRight, se, yMax) : fillRegion(0, X_MIN, zCritRight, se, yMax)} fill="#bfdbfe" fillOpacity={regionOpacity("one_minus_alpha")} style={{ transition: "fill-opacity 0.2s" }}/>

        {/* alpha region(s) under H0 */}
        <polygon points={fillRegion(0, zCritRight, X_MAX, se, yMax)} fill="#f87171" fillOpacity={regionOpacity("alpha")} style={{ transition: "fill-opacity 0.2s" }}/>
        {twoTailed && (<polygon points={fillRegion(0, X_MIN, zCritLeft, se, yMax)} fill="#f87171" fillOpacity={regionOpacity("alpha")} style={{ transition: "fill-opacity 0.2s" }}/>)}

        {/* beta region under H1 */}
        <polygon points={fillRegion(muAlt, twoTailed ? zCritLeft : X_MIN, zCritRight, se, yMax)} fill="#fbbf24" fillOpacity={regionOpacity("beta")} style={{ transition: "fill-opacity 0.2s" }}/>

        {/* power region under H1 */}
        <polygon points={fillRegion(muAlt, zCritRight, X_MAX, se, yMax)} fill="#4ade80" fillOpacity={regionOpacity("power")} style={{ transition: "fill-opacity 0.2s" }}/>
        {twoTailed && (<polygon points={fillRegion(muAlt, X_MIN, zCritLeft, se, yMax)} fill="#4ade80" fillOpacity={regionOpacity("power")} style={{ transition: "fill-opacity 0.2s" }}/>)}
        ...
        {/* curves */}
        <polyline points={curvePoints(0, se, yMax)} fill="none" stroke="#2563eb" strokeWidth={2.5} opacity={curveOpacity("null")} style={{ transition: "opacity 0.2s" }}/>
        <polyline points={curvePoints(muAlt, se, yMax)} fill="none" stroke="#dc2626" strokeWidth={2.5} opacity={curveOpacity("alt")} style={{ transition: "opacity 0.2s" }}/>

        {/* curve labels */}
        <text x={xToSvg(0)} y={PAD.top - 6} textAnchor="middle" fontSize={11} fill="#2563eb" fontWeight="bold" opacity={curveOpacity("null")} style={{ transition: "opacity 0.2s" }}>H₀</text>
        <text x={xToSvg(muAlt)} y={PAD.top - 6} textAnchor="middle" fontSize={11} fill="#dc2626" fontWeight="bold" opacity={curveOpacity("alt")} style={{ transition: "opacity 0.2s" }}>H₁</text>

        {/* critical value line + arrow + label (right) */}
        <line x1={xToSvg(zCritRight)} y1={PAD.top} x2={xToSvg(zCritRight)} y2={PAD.top + PLOT_H} stroke="#7c3aed" strokeWidth={1.5} strokeDasharray="5,3"/>
        {/* arrow pointing right */}
        <line x1={xToSvg(zCritRight)} y1={arrowY} x2={arrowRightX - 2} y2={arrowY} stroke="#7c3aed" strokeWidth={1.5} markerEnd="url(#arrowR)"/>
        ...
        {twoTailed && (<>
            <line x1={xToSvg(zCritLeft)} y1={PAD.top} x2={xToSvg(zCritLeft)} y2={PAD.top + PLOT_H} stroke="#7c3aed" strokeWidth={1.5} strokeDasharray="5,3"/>
            <line x1={xToSvg(zCritLeft)} y1={arrowY} x2={arrowLeftX + 2} y2={arrowY} stroke="#7c3aed" strokeWidth={1.5} markerEnd="url(#arrowL)"/>
        ...
        <text x={xToSvg(zCritRight + (X_MAX - zCritRight) / 2)} y={PAD.top + PLOT_H - 8} textAnchor="middle" fontSize={11} fill="#b91c1c" fontWeight="bold" opacity={hovered && hovered !== "alpha" ? 0.2 : 1} style={{ transition: "opacity 0.2s" }}>
          α={fmt(twoTailed ? alpha / 2 : alpha)}
        </text>
        {twoTailed && (<text x={xToSvg(X_MIN + (zCritLeft - X_MIN) / 2)} y={PAD.top + PLOT_H - 8} textAnchor="middle" fontSize={11} fill="#b91c1c" fontWeight="bold" opacity={hovered && hovered !== "alpha" ? 0.2 : 1} style={{ transition: "opacity 0.2s" }}>
            α/2
          </text>)}
        <text x={twoTailed ? xToSvg((zCritLeft + zCritRight) / 2) : xToSvg((X_MIN + zCritRight) / 2)} y={PAD.top + PLOT_H - 8} textAnchor="middle" fontSize={11} fill="#1d4ed8" fontWeight="bold" opacity={hovered && hovered !== "one_minus_alpha" ? 0.2 : 1} style={{ transition: "opacity 0.2s" }}>
          1−α={fmt(1 - alpha)}
        </text>
        {beta > 0.02 && (<text x={xToSvg(Math.min(muAlt, zCritRight - se * 0.5))} y={PAD.top + PLOT_H - 24} textAnchor="middle" fontSize={11} fill="#b45309" fontWeight="bold" opacity={hovered && hovered !== "beta" ? 0.2 : 1} style={{ transition: "opacity 0.2s" }}>
            β={fmt(beta)}
          </text>)}
        {power > 0.02 && (<text x={xToSvg(zCritRight + (X_MAX - zCritRight) / 2)} y={PAD.top + 44} textAnchor="middle" fontSize={11} fill="#15803d" fontWeight="bold" opacity={hovered && hovered !== "power" ? 0.2 : 1} style={{ transition: "opacity 0.2s" }}>
            Power={fmt(power)}
          </text>)}
      </svg>
      ...
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginTop: 10, fontSize: 13 }}>
        {[
            ...
            return (<div key={i} onMouseEnter={() => s.key && setHovered(s.key)} onMouseLeave={() => setHovered(null)} style={{
      ...
      <table style={{ borderCollapse: "collapse", fontSize: 13, width: "100%" }}>
        ...
        <tbody>
          <tr>
            ...
            <Cell concept="alpha" hovered={hovered} setHovered={setHovered} title="False Positive" sub="Type I Error" prob={`α = ${fmt(alpha)}`} isError={true}/>
            <Cell concept="power" hovered={hovered} setHovered={setHovered} title="True Positive" sub="Correct Rejection" prob={`Power = ${fmt(power)}`} isError={false}/>
          </tr>
          <tr>
            <td style={{ ...td(), fontWeight: 700, background: "#f9fafb" }}>Do not reject H₀</td>
            <Cell concept="one_minus_alpha" hovered={hovered} setHovered={setHovered} title="True Negative" sub="Correct Retention" prob={`1−α = ${fmt(1 - alpha)}`} isError={false}/>
            <Cell concept="beta" hovered={hovered} setHovered={setHovered} title="False Negative" sub="Type II Error" prob={`β = ${fmt(beta)}`} isError={true}/>
          </tr>
        </tbody>
      </table>
...
function Cell({ concept, hovered, setHovered, title, sub, prob, isError }) {
    ...
    return (<td onMouseEnter={() => setHovered(concept)} onMouseLeave={() => setHovered(null)} style={{
            ...td(),
