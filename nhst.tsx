...
type Concept = "alpha" | "beta" | "power" | "one_minus_alpha";
...
function xToSvg(x: number) { return PAD.left + ((x - X_MIN) / (X_MAX - X_MIN)) * PLOT_W; }
function yToSvg(y: number, yMax: number) { return PAD.top + PLOT_H - (y / yMax) * PLOT_H; }

const xs = Array.from({ length: N_POINTS }, (_, i) => X_MIN + (i / (N_POINTS - 1)) * (X_MAX - X_MIN));

function fillRegion(mu: number, x1: number, x2: number, se: number, yMax: number) {
...
function curvePoints(mu: number, se: number, yMax: number) {
  return xs.map(x => `${xToSvg(x)},${yToSvg(normalPDF(x, mu, se), yMax)}`).join(" ");
}

// concept keys
const conceptMeta: Record<Concept, { label: string; color: string; bg: string; textColor: string }> = {
...
export default function NHSTExplainer() {
  ...
  function regionOpacity(concept: Concept) {
    if (!hovered) return 0.45;
    return hovered === concept ? 0.85 : 0.12;
  }
  function curveOpacity(which: "null" | "alt") {
    // which: "null" or "alt"
    if (!hovered) return 1;
  ...
  return (
    <div style={{ fontFamily: "system-ui, sans-serif", maxWidth: 760, margin: "0 auto", padding: 16 }}>
      ...
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginTop: 10, fontSize: 13 }}>
        ...
        ] as { key: Concept | null; label: string; value: string }[]).map((s, i) => {
...
function Cell({ concept, hovered, setHovered, title, sub, prob, isError }: {
  concept: Concept;
  hovered: Concept | null;
  setHovered: (concept: Concept | null) => void;
  title: string;
  sub: string;
  prob: string;
  isError: boolean;
